...
repo carries it on every story. The branch gave it a per-story reader,
`isReady()`; by the time it landed, main already read `blockedBy` through
`workPlan()` (missing ids, cycles and malformed lists included), so the reader
was dropped and every consumer here uses `workPlan().ready` instead. The CLAUDE.md table is unchanged and `prd-states.js`'s states
