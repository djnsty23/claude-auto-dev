...
`[measured 2026-09-08]` on this machine, read-only against three product repos. Product
names are withheld because this repo is public; they are referred to as **A**, **B**
and **C**, with no further description. Nothing below was typed into a chat as a credential
...
| — the product's own error/attempt tables, read by hand via SQL | 10 | client errors from one function; rate-limit rows; HTTP 401s from one cron |
| — Search Console or field CLS beacons, read from a dashboard | 5 | an indexing-coverage count; a p75 CLS on one route |
...
| **from a usage or analytics figure** | **0** | |
...
production measurement (a 90-day count of one row; a funnel count)
...
markdown). Live signals: Vercel Web Analytics (wired, dashboard-only), product tables
...
No Sentry; a crash reporter whose output is console-only. Web errors go into a
first-party `events` table whose base table is RLS insert-only; the readable
surface is a set of `usage_*` views via the management API with a per-product
management PAT held under one Doppler name. Vercel Analytics live, dashboard-only.
Scheduled jobs exist. `prd.json` exists. Read only in this session, as instructed.
...
outward, with A's secrets injected by its secret manager, so A's repo
...
Function names, message text and story ids are withheld because they name a
private product's features; the counts, spans and readings are unchanged.

| # | signal | count / 14 d | span | reading |
|---|---|---|---|---|
| 1 | server function F1, no code: an upstream-provider failure message (78 of 84 share it) | 84 | 08-31 → 09-07, continuous | **REAL.** A recurring upstream-provider outage class; an incident of this shape is on file from 08-16, no open story. |
| 2 | server function F2, no code: a rate-limit message saying it will retry | 36 | 08-25 → 09-07 | **REAL, regression.** A story closed this row class on 08-21 having established the rows are failures a user saw. Still written at ~2.5/day. The ledger's requiet rule exists for exactly this. |
| 3 | server function F3 / a vendor 429 code (message varies per row: Retry-After seconds) | 484 | 08-25 → 09-05 | **REAL.** The single shared vendor account is rate-limited for hours at a time; a closed story fixed the recovery path, not the capacity. Grouping by message would have shattered this into 480 groups; by code it is one. |
| 4 | F3, no code: a cooldown message that calls itself transient | 729 | 08-25 → 09-04 | **NOISE row** (duplicate of 3): an expected transient state written as a server error 729 times. Same class as another closed story. The story, if any, is "stop writing it". |
| 5 | F2 / `HTTP_429`: a still-limited message after retries (61 of 63) | 63 | 08-25 → 09-04 | **REAL** (user-facing failure after retries), overlapping 3's lockout windows. |
...
| 7 | server function F4: a create call failed | 6 | 08-27 → 08-30 | **UNSURE.** Low, inside 3's lockout days, nothing since. |
| 8 | server function F5 / `HTTP_429` | 5 | **45 minutes** on 08-30 | **NOISE → held after the fix.** One rate-limit burst. |
| 9/11 | client: a fetch that failed to send a request to the edge function (both groupings) | 16 / 17 | 08-25 → 09-07 | **UNSURE, likely residual of a story closed 08-14 as a CORS fix.** ~1.2/day continues; "failed to send" is the client's network error, so offline/blocked clients are a competing cause. Worth a story that names both. |
| 10/12 | client: "An error occurred processing your request" on one page | 16 / 13 | 08-31 → 09-01 (22 h) | **UNSURE.** Generic message; the by-message group spans 21.9 h and is held as a burst after the fix; the by-url group still passes. |
...
error, "Script error.", "Load failed", a recovered retry), 3 as bursts (0.1 h,
