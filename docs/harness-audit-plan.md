...
| C6 | No duplicate work | **BUILT 2026-09-08.** `fleet-overlap.js` scores a fourth signal on the files each live worktree has touched: committed work since the trunk merge-base, uncommitted (staged and unstaged), and untracked, intersected pairwise within a repo, with measured ledger paths excluded and printed. The story join is still out of scope | 0 live pairs sharing 3 or more changed files — **NOT MET at build time: 2 such pairs** (one at 6 shared paths, one at 3). Corrected down from 5: the first count was inflated by pairs whose shared paths came from commits BOTH worktrees hold — a review checkout is not a collision — and by counting a reviewer pair at 17 paths that was one session reading another's branch |
