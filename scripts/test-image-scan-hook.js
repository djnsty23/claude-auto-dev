...
const hook = path.resolve(__dirname, '..', 'hooks', 'user-prompt-image-scan.js');
...
// Performance budget: each run < 100 ms wall (includes node startup ~30-50 ms)
for (let i = 0; i < results.length; i++) {
    assert('case ' + (i + 1) + ' < 150 ms wall (' + results[i].ms.toFixed(1) + ' ms)', results[i].ms < 150);
}
