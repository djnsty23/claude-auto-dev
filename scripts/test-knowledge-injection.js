...
//
// The hook resolves memory-db from ~/.claude/scripts/, so HOME is redirected to a
// fresh temp dir and the repo's memory-db.js + semantic-search.js are copied there.
...
const HOOK = path.resolve(__dirname, '..', 'hooks', 'post-tool-typecheck.js');
const REPO_SCRIPTS = path.resolve(__dirname);

const TMP_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'knowinject-home-'));
...
const HOME_SCRIPTS = path.join(TMP_HOME, '.claude', 'scripts');
...
if (!memDB.isAvailable()) {
  ...
  function edit(relFile) {
    ...
    return spawnSync(process.execPath, [HOOK], {
      ...
      encoding: 'utf8',
      cwd: PROJ,
      env: { ...process.env, HOME: TMP_HOME, USERPROFILE: TMP_HOME, AUTO_DEV_SESSION_ID: SESSION }
    });
  }
  ...
  const r5 = spawnSync(process.execPath, [HOOK], {
    ...
    encoding: 'utf8',
    cwd: PROJ,
    env: { ...process.env, HOME: TMP_HOME, USERPROFILE: TMP_HOME, AUTO_DEV_SESSION_ID: 'test-sess-2' }
  });
