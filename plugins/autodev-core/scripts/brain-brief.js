...
async function sectionWork(repos) {
    ...
    say('');
    say('  Before treating a count here as lost work, ask about CONTENT:');
    say('    node ${CLAUDE_PLUGIN_ROOT}/scripts/check-branch-landed.js --repo <worktree>');
    say('');
    say('  NOT `git cherry`, and NOT `rev-list --left-right`. This line recommended');
    say('  `cherry` until 2026-09-07, while skills/brain/SKILL.md had already said for');
    say('  weeks that cherry CANNOT answer this - the skill was corrected and the');
    say('  script that ships beside it was not, so the tool kept teaching the probe the');
    say('  documentation had retired. Both of them measure ANCESTRY:');
    say('    - cherry compares PATCH IDS, which a squash rewrites, so every original');
    say('      commit still reads + (absent upstream) after it has landed;');
    say('    - a squash-merged branch is never an ancestor of the trunk, so');
    say('      `rev-list --left-right` reports it ahead FOREVER;');
    say('    - `diff --shortstat A...B` (three dots) counts the branch\'s whole');
    say('      contribution since the merge base, landed or not.');
    say('  [measured 2026-09-07] those three reported 3 branches as carrying 213, 773');
    say('  and 1033 insertions of unlanded work. All three were already merged, as');
    say('  PRs #168, #163 and #161, and a session was dispatched to re-land them.');
    say('');
    say('  What settles it is a MERGED PR WHOSE headRefOid EQUALS THE BRANCH TIP,');
    say('  whose merge commit is an ancestor of the trunk - looked up BY COMMIT SHA,');
    say('  never by `gh pr list --search`, whose empty result is a claim about the');
    say('  query and not about the world. Resolve origin/HEAD rather than assuming');
