...
'use strict';
/**
 * mission-store.js — durable, fenced mission state for Brain (backlog item B05).
 *
 * WHAT IT IS. One SQLite file per store, private to the user (0700/0600), holding
 * missions, attempts, results, launches, bootstrap permits, an outbox and an
 * event log. Every command is one transaction keyed by a caller-supplied
 * eventId: replaying an event returns its first answer, and replaying it with a
 * different payload is refused (`event-conflict`). Ownership is a fenced
 * (owner, generation, attemptId) triple; a stale owner can report but cannot
 * settle. Attempts carry bounded backoff and exhaustion. A worktree holds one
 * live reservation. The record survives a restart: `status` reconstructs
 * attempts, results, launches, bootstraps, outbox and events.
 *
 * WHAT IT IS NOT. A claim is a database reservation, not proof of a subprocess.
 * Envelope acceptance validates declared metadata only, never execution or
 * artifacts; every answer carries `verified: false`. No scheduler, no model, no
 * transport: dispatch (B06), delivery (B07) and independent verification are
 * separate work. Private same-user storage is a trust boundary; owner names
 * are not credentials.
 *
 * HOST. Requires node:sqlite and POSIX ownership checks; anywhere else every
 * command answers `runtime-unavailable` and creates nothing.
 *
 * PROVENANCE. Adopted unchanged in behaviour from the overnight audit's
 * fixture-only prototype (docs/drafts/brain-live-audit/mission-runtime-v3-draft/,
 * reviewed there with mutation controls); its suites are tooling/test-mission-*.js.
 * Build an admit payload from a prd.json story with mission-contract.js.
 */
const USAGE = [
  'Usage: node mission-store.js <command> --store /absolute/private/dir  < payload.json',
  'Commands: init status admit claim fail receive accept-envelope prepare-start',
  '          authorize-bootstrap register-executor observe-worker poll-worker',
  '          reject-result enqueue-result begin-delivery ack-delivery',
  'Input:  one JSON object on stdin ({} for init); every command except init and',
  '        status carries an eventId, and a replayed eventId returns its first answer.',
  'Output: {"ok":true,"value":...} exit 0, or {"ok":false,"error":{"code","message"}} exit 1.',
  'Host:   needs node:sqlite and POSIX ownership checks; otherwise runtime-unavailable.',
  'Build the admit payload for a prd.json story with mission-contract.js.',
].join('\n') + '\n';
...
function transact(db, command, input) {
  ...
  try {
    ...
    if (prior) {
      ...
      if (command === 'authorize-bootstrap') fault('bootstrap-already-issued');
