...
Why this form and not "escalate always" or "add a canary": the evidence doc
behind this decision was withdrawn for privacy, because it described another
repository's deployments. The short version: `[measured 2026-09-08]` on a repo
where a merge to main is the production deploy, most deploy-caused incidents
came through a hand-run path with no record of tree, branch, lock or gate. The
ledger row is what those lacked; escalating always would have cost hours of wait
per deploy and reversed two of his own rules.
