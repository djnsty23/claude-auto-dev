...
| Feature (UI) | Build passes + visual check for every changed UI + complete the primary user flow once |
| Edge Function / API | Exercise the local/preview endpoint with real parameters; verify expected response, authorization and side effects. Production deploy follows `ship` |
