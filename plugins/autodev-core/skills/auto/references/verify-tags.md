...
| `flow` | Drive the primary flow the acceptance criterion describes in the browser, read the outcome back as a value, write `.claude/evidence/<story>/flow.json` with the `commit` the dev server was serving (`git rev-parse HEAD` when the flow was driven), and pass it through `scripts/flow-evidence.js` — exit 0 closes the story; a record with no assertion, or whose `commit` is not HEAD or an ancestor of it, is refused (SKILL.md, "Runtime flow check") |
