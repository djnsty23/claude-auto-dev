# Historical local-first procedure — snapshot 9 September 2026

This is preserved evidence from the earlier procedure. Dated machine details,
authorization, tool limits and workarounds are not current policy. Load only to
investigate the recorded incidents; the parent SKILL.md owns the active workflow.
...
Added 2026-08-21, on the operator's instruction: *"always test locally and run visual checks in
...
**A gate script this rule names may not exist.** `[measured]` 2026-08-22: a product repo
this line once said had a `gate` script had none, on its working branch or on `origin/main`,
so its gate was a hand-typed chain such as `npm run typecheck && npm run test && npm run build`.
Where `build` compiles only some tsconfigs, `typecheck` is the step that catches an `api/`
change that builds clean and is still broken.
...
the port out of the app's own config rather than assuming a framework default. An app can
serve on a non-default port, and a preview pointed at the wrong port fails in a way that
looks like a broken app.

## Pushing

Commits stay local. An ad-hoc `git push`, PR or merge needs the operator to say so in that turn.
"It is ready to push" is a status line, not a licence.

The batched publish below is the one exception, and only halfway: the operator set the
*trigger*, so you do not re-ask whether batching is allowed. You still present the manifest
and get a yes before it goes, because these repos deploy on push (Fly, Pages, release gates) and
`agent-quality.md` rule 4 does not bend for a cadence. The win is one confirmation per
batch instead of one per commit.

This does not cancel `agent-quality.md` rule 8. That rule covers the case where the operator
says push, and it still means do it without re-asking. What changed is the default ending of
...
The operator, 2026-08-21: *"what if we commit once per day as a daily publish or when we queue
...
in every repo checked (four product repos), and
...
Pick the port explicitly and check it first. Other sessions hold ports: two were already
taken, by another chat's dev server and by the fleet board. `preview_start` refuses
...
**Which repo has which gate script** varies by repo, and one repo documented as having a
`gate` script had none (see the gate section above). Read `package.json` rather than this
list.

**Ports** differ per app. Read the port from the app's own config, because a preview
pointed at a framework default fails in a way that looks like a broken app.

**`.claude/` is gitignored** in every one of the four product repos checked, which is why
`publish-queue.md` belongs at the repo ROOT. A queue inside `.claude/` is never committed,
so it is invisible to a session on another machine: the exact failure the publishing
section warns about, reintroduced by its own filename.
...
`[measured]` 2026-08-22, in a product repo. A rect-based sweep flagged four controls on one
page as under the 44px minimum. All four were fine. The project defines an `::after`
utility that expands the hit area to 44x44 under `@media (pointer: coarse)`, and many
...
Then print the population, not a verdict: *"N undersized visual boxes, M covered by the
expansion utility, the rest not, and where the rest sit"* cannot be misread the way a bare
count can. And before prescribing a fix, read what the existing utility says it costs. This
one documents that expanded regions overlap in clusters tighter than 8px, which makes it the
wrong tool for a dense footer.

## Do not switch branches in a clone another session is using

`[measured]` 2026-08-22. I checked out five branches in a product repo's main clone over one
session. It was not idle: uncommitted work on source and test files appeared in that tree
while I worked, from another session. Uncommitted changes
...
That repo carries many worktrees under `.claude/worktrees/` precisely so parallel sessions
