...
// The title is a basename, so two different files can share it. Merged with
// #226, which keeps distinct source contexts apart: the repeat rule keys on
// source_files too, so a same-named file elsewhere in the project is a new row.
const otherFile = memDB.saveObservation({
  sessionId: sesA, projectPath: PROJ, type: 'change', title: 'Modified x.js',
  concept: `edit number ${++n}`, sourceFiles: [path.join(PROJ, 'lib', 'x.js')],
});
check('control: the same title for a different source file is recorded', typeof otherFile === 'string');
check('  and the count is five', count() === 5);

// Only capture-shaped `change` rows are collapsed. A deliberate decision saved
// twice under one title with different concepts is a conflict to keep, not a
// repeat (test-memory-knowledge-conflicts.js depends on both surviving).
const decA = memDB.saveObservation({ sessionId: sesA, projectPath: PROJ, type: 'decision', title: 'Config decision', concept: 'Enable', sourceFiles: ['c.js'] });
const decB = memDB.saveObservation({ sessionId: sesA, projectPath: PROJ, type: 'decision', title: 'Config decision', concept: 'Disable', sourceFiles: ['c.js'] });
check('control: a conflicting decision under the same title both survive', typeof decA === 'string' && typeof decB === 'string');
check('  and the count is seven', count() === 7);
