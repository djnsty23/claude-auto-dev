...
// 2026-09-07. --json here carries one record per script, so it grows with the
...
// on every platform.
{
    ...
    const files = {};
    for (let i = 0; i < 400; i++) {
        files['tooling/check-a-fairly-long-orphan-name-' + String(i).padStart(4, '0') + '.js'] = 'assert(1);\n';
    }
    const dir = repo(files);
