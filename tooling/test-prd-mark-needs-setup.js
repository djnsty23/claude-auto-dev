...
// Dependency readiness through the shared planner auto and the Stop hook use,
// not a private predicate that could disagree with it.
const readyIds = (stories) => S.workPlan({ stories }).ready.map(([id]) => id);
...
// ---------------------------------------------------------------- marks
{
    ...
    check('a dependent story is not ready while its dep is blocked on a human', !readyIds(st).includes('S1-005'), readyIds(st));
    check('workPlan.ready excludes the dependent', readyIds(st).length === after.actionable - 1, readyIds(st));
...
// ---------------------------------------------------------------- clear
{
    ...
    check('clear: the dependent is still not ready (dep is pending, not done)', !readyIds(st).includes('S1-005'), readyIds(st));
    ...
    check('  ...and the dependent becomes ready once the dep is closed', readyIds(st).includes('S1-005'), readyIds(st));
}
{
    const f = fixture(FLAT());
    const orig = fs.readFileSync(f, 'utf8');
