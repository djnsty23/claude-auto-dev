...
try {
    ...
    // this suite nested worktree paths ten levels deep with 60-
    // character segments to reach it, running to ~700 characters, and
    // `git worktree add` failed with "Filename too long" on windows-latest
    ...
    {
        // One ordinary repo, one worktree. No deep nesting and no long
        // segments: with the pipe pre-filled, one row truncates as visibly
        // as fifty-seven did.
        const drainBase = fs.mkdtempSync(path.join(os.tmpdir(), 'dispatch-pipe-'));
        const drainRepo = path.join(drainBase, 'repo');
        fs.mkdirSync(drainRepo);
        const g = (a) => execFileSync('git', a, { cwd: drainRepo, encoding: 'utf8', stdio: 'pipe', windowsHide: true });
        g(['init', '-q', '-b', 'main']);
        g(['config', 'user.email', 't@t']);
        g(['config', 'user.name', 'T']);
        fs.writeFileSync(path.join(drainRepo, 'seed.txt'), 'seed\n');
        g(['add', '.']);
        g(['commit', '-qm', 'seed']);

        const drained = require('./pipe-drain').run({ argv: [SUBJECT, drainRepo, '--json'] });
        ok('--json arrives whole through a stalled pipe', drained.ok, drained.detail);

        fs.rmSync(drainBase, { recursive: true, force: true, maxRetries: 3 });
    }
